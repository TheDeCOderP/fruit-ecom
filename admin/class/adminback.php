...
class  adminback
{
    ...
    {
        $query = "SELECT * FROM `products` WHERE pdt_id=$pdtId";
        if (mysqli_query($this->connection, $query)) {
            $pdt_info = mysqli_query($this->connection, $query);
            return $pdt_info;
        }
    ...
    {
        $query = "SELECT * FROM `products` WHERE `product_stock`>1 ";
    ...
    {
        $query = "SELECT * FROM `products` WHERE `pdt_price`>200  ORDER BY `pdt_price` LIMIT 12";
    ...
    {
        $query = "SELECT * FROM `order_details` ORDER BY `order_time` DESC";
